...
void iface_send_packet(iface_info_t *iface, const char *packet, int len)
{
	...
	struct ether_header *eh = (struct ether_header *)packet;	//以太网帧头部
	memcpy(addr.sll_addr, eh->ether_dhost, ETH_ALEN);	//加上以太网帧头部的目的主机MAC地址
...
void init_ustack()
{
	...
	bzero(instance, sizeof(ustack_t));	//将内存块（字符串）的前n个字节清零
	init_list_head(&instance->iface_list);	//iface_list是一个双向链表
